      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['warnInfo']"
                  :label="get4Label('告警信息')"
                  name='warnInfo'
                  prop='warnInfo'
                  placeholder="请选择告警信息"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <online-select
                :label="get4Label('告警类别')"
                 labelWidth="100px"
                 type="list"
                 name='deviceType'
                 dict="device_type"
                v-model="myFormData['deviceType']"
              ></online-select>
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['deviceId']"
                  :label="get4Label('设备id')"
                  name='deviceId'
                  prop='deviceId'
                  placeholder="请选择设备id"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <DateTime
                  :label="get4Label('告警时间')"
                  labelWidth="100px"
                  type="datetime"
                  format="YYYY-MM-DD HH:mm:ss"
                  name='warnTime'
                  v-model="myFormData['warnTime']"
              ></DateTime>
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['deviceName']"
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['insLocation']"
                  :label="get4Label('点位')"
                  name='insLocation'
                  prop='insLocation'
                  placeholder="请选择点位"
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['lineLocation']"
                  :label="get4Label('线路位置')"
                  name='lineLocation'
                  prop='lineLocation'
                  placeholder="请选择线路位置"
                  :rules="[
...
const dataId = ref('')
const backRouteName = ref('FtuWarnInfoList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/ftu/ftuWarnInfo/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/ftu/ftuWarnInfo/edit':'/ftu/ftuWarnInfo/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
