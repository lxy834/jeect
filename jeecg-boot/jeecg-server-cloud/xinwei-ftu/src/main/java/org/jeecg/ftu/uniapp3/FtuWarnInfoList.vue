...
<template>
  <PageLayout navTitle="告警记录"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './FtuWarnInfoData';
defineOptions({
  name: 'FtuWarnInfoList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/ftu/ftuWarnInfo/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/ftu/ftuWarnInfo/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
